#!/usr/bin/env python3
"""
aether.py · the STANDALONE aether-kit CLI (not the ecosystem wrapper)
=====================================================================

⚠ READ THIS BEFORE INVOKING ⚠

This file is NOT what `aether <cmd>` on your shell calls. The global
`aether.bat` / `aether` wrapper installed by `aether_install.py` routes
subcommands to sibling scripts (aether_project.py, aether_tasks.py,
aether_daily.py, …). See `_generate_wrapper_content` in aether_install.py.

THIS file is the independent open-source "aether-kit" CLI published at
https://github.com/497810wsl/aether-kit · v0.3.0 · MIT.

It speaks a different vocabulary (fields · collapses · species · nursery)
targeted at third-party users who want to bootstrap an `.aether/`
workspace inside any project. Subcommands:

    python aether.py init [--integration cursor|claude|generic] [--force]
    python aether.py fetch <field-id> [--from <url>]
    python aether.py list [--type style|discipline|temperament|all]
    python aether.py collapse "<summary>" --fields "id1=0.8,id2=0.6"
    python aether.py status
    python aether.py link
    python aether.py doctor     # ← DIFFERENT from `aether doctor` (wrapper)
    python aether.py version

If you typed `aether <cmd>` in your shell and got here, you're on the
WRONG path. Use the wrapper instead · run `aether --help` for the real
ecosystem subcommand list.

This file is intentionally isolated from the ecosystem scripts · it has
no imports from aether_paths / aether_events / aether_hook etc. Keeping
it dependency-free lets us eventually ship `aether-kit` as a separate
package without dragging in the Owner's federated-memory stack.

License: MIT · https://github.com/497810wsl/aether-kit
...
- <https://github.com/497810wsl/aether-kit>
- FIELDS.md and PRESETS.md in the Aether Kit repo
...
- Core: <https://github.com/497810wsl/aether-kit>
- Contact: WeChat **wsl497810**
...
FIELD_LOCATIONS = [
    "gen4-morphogen/fields/style-fields/{fid}.field.md",
    "gen4-morphogen/fields/discipline-fields/{fid}.field.md",
    "gen4-morphogen/fields/temperament-fields/{fid}.field.md",
]
